...
try {
  ...
  assert((await page.locator("#globalSearch").getAttribute("role")) === "combobox", "global search should expose combobox role");
  assert((await page.locator("#globalSearch").getAttribute("aria-expanded")) === "true", "global search aria-expanded should open with suggestions");
  await page.keyboard.press("ArrowDown");
  assert((await page.locator(".search-suggestion.active").count()) === 1, "ArrowDown should focus one suggestion");
  await page.keyboard.press("Escape");
  assert((await page.locator("#globalSearchSuggestions").isHidden()) === true, "Escape should close suggestions");
  await page.fill("#globalSearch", "233");
  await page.waitForSelector("#globalSearchSuggestions:not([hidden])");
  await page.keyboard.press("ArrowDown");
  await page.keyboard.press("Enter");
  await page.waitForFunction(() => document.querySelector("#stockNo")?.value === "2330");
  await page.fill("#globalSearch", "MOS");
  await page.waitForSelector("#globalSearchSuggestions:not([hidden])");
